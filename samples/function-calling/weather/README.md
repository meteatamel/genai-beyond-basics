...
In this sample, you'll learn how to use function calling for multiple and sometimes parallel functions 
through the sample [main.py](./main.py). You'll also see how you can use function calling from `generate_content` 
method and from the chat interface. We'll also take a look at the auto function calling feature.
...
Then a geocoding function called `location_to_lat_long`. Note how we properly documented the function. This will be 
important later when we do function declarations for the LLM:
...
Next, let's define a weather function called `lat_long_to_weather`. Again note the proper documentation for the LLM:
...
Now that the two functions are ready, let's now create a model with a tool where we describe each function. We're using
`FucntionDeclaration.from_func` method to get a function declaration for the LLM, combine these function declarations
in a tool and create a model with the tool.
...
We're ready to generate some content but let's talk about function calls first.
...
We also have a helper method to handle function calls. It looks up the name in the function call request and calls the
actual function in our code:
...
## Run generate content with function calls
...
There's a new feature in `chat` where the library automatically calls your functions, so you don't have to. This greatly
simplifies the code.

First, import the preview libraries:
...
function calls you'll need but once that's specified, there's nothing else you need to do in your code. Much simpler!
...
## Run chat with auto function calls
